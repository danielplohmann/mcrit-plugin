# Releasing the MCRIT IDA plugin
...
The version is declared in `ida-plugin.json` (`plugin.version`) and `config.py` (`VERSION`); `verify_metadata_sync.py` also
...
2. In one commit on a branch, then merged through a PR:
   - set the new version in `ida-plugin.json` (`plugin.version`) and `config.py` (`VERSION`); `verify_metadata_sync.py` also
requires the newest `CHANGELOG.md` heading to agree;
...
3. Wait for CI to pass on the merge commit. Then tag that commit and push the tag:
   ...
   ```bash
   git tag -a vX.Y.Z -m "the MCRIT IDA plugin X.Y.Z"
...
2. **Validate and build** — runs `verify_metadata_sync.py` against the tag's version,
   ...
   the repository and the archive with `hcli plugin lint`.
3. **Release** — creates the GitHub release for the tag with the changelog section as its body,
   GitHub's generated contributor and PR list appended under it, and the archive attached. The
...
`ida-plugin.json` (`plugin.version`) and `config.py` (`VERSION`); `verify_metadata_sync.py` also
requires the newest `CHANGELOG.md` heading to agree and a `## [X.Y.Zrc1] - YYYY-MM-DD` changelog section. The workflow marks the
...
```bash
python scripts/verify_metadata_sync.py --repo . --expected-version X.Y.Z
python scripts/verify_settings_sync.py --repo .
python scripts/run_quality_checks.py --repo .
python scripts/package_plugin.py --repo . --output dist/mcrit-ida-X.Y.Z.zip
hcli plugin lint . && hcli plugin lint dist/mcrit-ida-X.Y.Z.zip
```
...
The plugin vendors a minimal MCRIT client (`helpers/minimcrit/`) rather than importing the `mcrit`
package, so it is not released in lockstep with MCRIT; it talks to the server over the REST API.
It does need `smda` in IDA's interpreter (`pythonDependencies` in `ida-plugin.json`), so when a
